# Contributing to Andiora Packages

This document captures the architectural conventions and best practices
developed for Andiora APKG packages. Follow these when adding or modifying
packages.
...
```xml
<PackageName>andiora-software-properties-common</PackageName>
...
Examples: `andiora-software-properties-common`, `andiora-software-properties-gtk`.
...
<PackageVersion>1:13.5+andiora1</PackageVersion>
```
...
```xml
<Dependency Include="andiora-software-properties-gtk" Condition="'$(Suite)' == 'resolute-addon'" />
```
...
When you need distro identity files (like `andiora.info` → `ubuntu.info`),
use relative symlinks with explicit runtime dependencies:

```xml
<PrebuildCommand Run="for d in obj/*; do mkdir -p &quot;$d/usr/share/python-apt/templates&quot; &amp;&amp; ln -sf ubuntu.info &quot;$d/usr/share/python-apt/templates/andiora.info&quot;; done" />
...
<IncludeFile Include="dconf/14-dash-to-panel.conf"
             Target="/etc/dconf/db/andiora.d/14-dash-to-panel.conf" />
```
...
is automatically recompiled by the dpkg trigger in `andiora-dconf-runtime`.
...
- **Different UUID, same function**: Use P-C-R at the meta-package level
  (`andiora-gnome-extensions` provides `gnome-shell-ubuntu-extensions`).
...
- Fork packages: `$(UpstreamVersion)+andiora1` — tracks upstream version
